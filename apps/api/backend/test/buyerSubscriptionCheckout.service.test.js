...
test("buyer checkout uses a stable scoped idempotency key", async () => {
  const keys = [];
  const prismaClient = { user: { findUnique: async () => ({ id: "buyer-1", name: "Buyer", email: "buyer@example.test", stripeCustomerId: "cus_1" }) } };
  const stripeClient = {
    ...
    checkout: { sessions: { create: async (_params, options) => { keys.push(options.idempotencyKey); return { url: "https://checkout.stripe.test/session" }; } } },
  };
