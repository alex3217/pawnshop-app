...
function fixture({ subscriptionStatus = "ACTIVE", methods = null } = {}) {
  const user = { id: "user_1", name: "Buyer", email: "buyer@example.test", stripeCustomerId: "cus_owned", buyerSubscription: { status: subscriptionStatus, stripeCustomerId: "cus_owned" } };
  ...
  const consents = [];
  const stripe = {
    customers: { retrieve: async () => ({ id: "cus_owned", invoice_settings: { default_payment_method: "pm_owned" } }), create: async () => ({ id: "cus_created" }), update: async (...args) => { updates.push(args); } },
    ...
    billingPortal: { sessions: { create: async () => ({ url: "https://billing.stripe.com/session" }) } },
  };
