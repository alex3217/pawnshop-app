...
export async function syncBuyerSubscriptionEvent({ event, prismaClient = prisma }) {
  ...
  const billingProfile = String(ref.metadata.billingProfile || "").trim().toUpperCase();
  if (billingProfile && billingProfile !== "BUYER") return { handled: false, applied: false };
  const lockSubject = ref.subscriptionId || ref.userId || event.id;

  return runBuyerAtomicTransaction({
    prismaClient,
    lockKey: `buyer-subscription-webhook:${lockSubject}`,
    operation: async (transaction) => {
      if (transaction.buyerSubscriptionEvent) {
        const duplicateAudit = await transaction.buyerSubscriptionEvent.findUnique({ where: { stripeEventId: event.id } });
        if (duplicateAudit) return { handled: true, applied: duplicateAudit.applied, duplicate: true, audit: duplicateAudit };
      }
      let existing = ref.subscriptionId ? await transaction.buyerSubscription.findUnique({ where: { stripeSubscriptionId: ref.subscriptionId } }) : null;
      if (!existing && ref.userId && billingProfile === "BUYER") existing = await transaction.buyerSubscription.findUnique({ where: { userId: ref.userId } });
      if (!existing && ref.userId && ref.subscription && billingProfile === "BUYER") {
        const planCode = String(ref.metadata.planCode || "").toUpperCase();
        if (!PLANS.has(planCode)) return { handled: true, applied: false };
        existing = await transaction.buyerSubscription.create({ data: {
      ...
      if (!isNewer(event, existing)) {
        ...
        const auditRow = await audit(transaction, event, existing, false, duplicate ? "DUPLICATE" : "OUT_OF_ORDER");
      ...
      const updated = await transaction.buyerSubscription.update({ where: { id: existing.id }, data: {
      ...
      const auditRow = await audit(transaction, event, updated, true, "APPLIED");
