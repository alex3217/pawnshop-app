...
import {
  ...
  normalizeAuctionForResponse as normalizeBaseAuctionForResponse,
  resolveCreateStatus,
...
function normalizeAuctionForResponse(auction, now = new Date()) {
  ...
  const response = normalizeBaseAuctionForResponse(auction, now);

  if (response._count) {
    response.bidCount = Number(response._count.bids || 0);
