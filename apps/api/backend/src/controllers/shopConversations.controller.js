...
export async function searchShopMessageRecipients(req, res) {
  try {
    ...
    const rows = await prisma.user.findMany({ where: { isActive: true, messageDiscoverable: true, publicDisplayName: { not: null }, blockedMessagingShops: { none: { shopId } }, AND: [{ OR: [{ allowShopFirstContact: true }, relationshipWhere(shopId)] }, { OR: [{ publicDisplayName: { contains: query, mode: "insensitive" } }, { publicMessageIdentifier: { contains: query, mode: "insensitive" } }] }] }, take: 20, orderBy: { publicDisplayName: "asc" }, select: { publicDisplayName: true, publicMessageIdentifier: true } });
...
export async function postMessage(req, res) {
  try {
    ...
    if (side === "SHOP" && !conversation.recipientShopId) {
      ...
      if (!consent?.isActive || !consent.allowTransactionalMessages || consent.blockedMessagingShops.length) throw httpError(403, "This buyer is not accepting messages from your shop.", "RECIPIENT_UNAVAILABLE");
    }
    const result = await prisma.$transaction(async (tx) => {
