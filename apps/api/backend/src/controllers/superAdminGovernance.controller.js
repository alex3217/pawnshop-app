...
export async function lookupSuperAdminUsers(req, res) {
  ...
  const where = lookupType === "EMAIL" ? { email: { equals: query, mode: "insensitive" } } : lookupType === "INTERNAL_ID" ? { id: query } : { OR: [{ name: { contains: query, mode: "insensitive" } }, { publicMessageIdentifier: { contains: query, mode: "insensitive" } }] };
  const users = await prisma.user.findMany({ where, take: lookupType === "PUBLIC" ? 25 : 5, orderBy: { createdAt: "desc" }, select: { id: true, name: true, email: true, publicMessageIdentifier: true, isActive: true, role: true } });
  ...
  return res.json({ success: true, users: users.map((user) => ({ publicDisplayName: user.name, pawnLoopIdentifier: user.publicMessageIdentifier, privateEmail: user.email, internalId: user.id, accountStatus: user.isActive ? "ACTIVE" : "SUSPENDED", role: user.role })) });
}

export async function getUserGovernance(req, res) {
  const user = await prisma.user.findUnique({ where: { id: req.params.id }, include: { governanceRestriction: true, shops: { select: { id: true, name: true, isActive: true, isDeleted: true } }, staffMemberships: { select: { id: true, role: true, status: true, shop: { select: { id: true, name: true } } } }, shopConversations: { select: { id: true, status: true, blockedAt: true, blockedByUserId: true, moderationState: true, _count: { select: { abuseReports: true } } } } } });
  if (!user) throw error(404, "User not found.");
  const admin = restrictionState(user);
  const blockedCount = user.shopConversations.filter((row) => row.status === "BLOCKED").length;
  const effective = resolveEffectiveMessagingPermission({ userActive: user.isActive, userConsent: false, blocked: blockedCount > 0, administrativeRestriction: admin.messagingRestricted, contextAuthorized: false });
  ...
  return res.json({ success: true, user: { publicDisplayName: user.name, pawnLoopIdentifier: user.publicMessageIdentifier, privateEmail: user.email, internalId: user.id, role: user.role, ...admin, messagingEligibility: effective, publicDiscoverability: { effective: user.isActive && !admin.discoverabilityRestricted, administrativeRestriction: admin.discoverabilityRestricted }, firstContactConsent: { state: "DEPENDENCY_PENDING", source: "BUYER_MESSAGING_PROFILE" }, administrativeRestrictions: user.governanceRestriction, shops: user.shops, memberships: user.staffMemberships, blockingAndReports: { blockedConversationCount: blockedCount, reportCount: user.shopConversations.reduce((sum, row) => sum + row._count.abuseReports, 0), conversations: user.shopConversations } } });
}
