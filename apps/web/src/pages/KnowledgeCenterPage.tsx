...
export default function KnowledgeCenterPage() {
  ...
  return <main className="training-shell">
    ...
    <form className="training-filters" onSubmit={submit} aria-label="Knowledge filters">
      ...
      <select aria-label="Content type" value={filters.type} onChange={(e) => setFilters({ ...filters, type: e.target.value })}><option value="">All formats</option><option>VIDEO</option><option>TUTORIAL</option></select><button type="submit" disabled={loading}>{loading ? "Searching…" : "Search"}</button>
    </form>
    {loading ? <p role="status">Loading knowledge content…</p> : error ? <div role="alert" className="training-error"><p>{error}</p><button type="button" onClick={() => void load()}>Try again</button></div> : !items.length ? <div className="training-empty" role="status"><h2>No lessons found</h2><p>No published lessons are available for your account and current filters. Try a broader search or check back after new content is published.</p></div> : <section className="training-grid" aria-label="Training lessons">{items.map((item) => <article className="training-card" key={item.id}>
