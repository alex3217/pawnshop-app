...
const outDir = "reports/owner-inventory-inspect";
...
async function inspectTheme(theme, token) {
  ...
  const result = await page.evaluate(() => {
    ...
    const els = Array.from(
      document.querySelectorAll(
        "a, button, input, select, textarea, article, section, div, h1, h2, h3, p, span, small, strong,label",
      ),
    );
  ...
  fs.writeFileSync(
    path.join(outDir, `owner-inventory-${theme}.json`),
  ...
  console.log(JSON.stringify(result.slice(0, 160), null, 2));
  console.log(`Saved: ${outDir}/owner-inventory-${theme}.json`);
  console.log(`Saved: ${outDir}/owner-inventory-${theme}.png`);
