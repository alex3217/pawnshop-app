...
const outDir = "reports/owner-staff-inspect";
...
async function inspectTheme(theme, token) {
  ...
  await page.goto(`${WEB}/owner/staff?fresh=${Date.now()}`, {
  ...
  const result = await page.evaluate(() => {
    ...
    const els = Array.from(document.querySelectorAll("a, button, input, select, textarea, article, section, div, h1, h2, h3, p, span, small, strong,label"));
  ...
  fs.writeFileSync(
    path.join(outDir, `owner-staff-${theme}.json`),
    JSON.stringify(result, null, 2),
  );

  await page.screenshot({
    path: path.join(outDir, `owner-staff-${theme}.png`),
    fullPage: true,
  });

  console.log(`\n===== ${theme.toUpperCase()} MODE =====`);
  console.log(JSON.stringify(result.slice(0, 120), null, 2));
  console.log(`Saved: ${outDir}/owner-staff-${theme}.json`);
  console.log(`Saved: ${outDir}/owner-staff-${theme}.png`);
