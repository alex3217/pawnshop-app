...
const outDir = "reports/owner-subscription-control-audit";
...
async function inspectTheme(theme, token) {
  ...
  await page.goto(`${WEB}/owner/subscription?fresh=${Date.now()}`, {
  ...
  const result = await page.evaluate((requiredTexts) => {
    ...
    return {
      url: location.href,
      bodyPreview: bodyTextRaw.slice(0, 3500),
      missingTexts,
  ...
  await page.screenshot({
    path: path.join(outDir, `owner-subscription-${theme}.png`),
    fullPage: true,
  });
