...
boolean_settings=(INVITE_ONLY_REGISTRATION_ENABLED AUTH_RATE_LIMIT_ENABLED CORS_ALLOW_PAWNLOOP_PREVIEWS SMTP_SECURE AUCTION_SCHEDULER_ENABLED MARKETPLACE_RESERVATION_SCHEDULER_ENABLED)
for name in "${boolean_settings[@]}"; do
  use_fixture staging-valid.env; set_value "$name" yes; expect_fail "malformed boolean $name"
done
for name in INVITE_ONLY_REGISTRATION_ENABLED AUTH_RATE_LIMIT_ENABLED CORS_ALLOW_PAWNLOOP_PREVIEWS; do
