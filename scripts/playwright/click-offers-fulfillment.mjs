...
const OUT = process.env.OUT || `reports/offers-fulfillment-click-${Date.now()}`;
...
await page.goto(`${WEB_BASE}/offers`, { waitUntil: "networkidle" });
...
if (!(await readyButton.count())) {
  throw new Error("No Ready for pickup button found on /offers.");
}
...
if (!(await completedButton.count())) {
  throw new Error("No Mark completed button found on /offers.");
}
